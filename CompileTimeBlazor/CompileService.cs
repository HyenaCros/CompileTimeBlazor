...
namespace CompileTimeBlazor
{
    public class CompileService
    {
        ...
        public async Task<Type> CompileBlazor(string code)
        {
            ...
            var engine = RazorProjectEngine.Create(RazorConfiguration.Create(RazorLanguageVersion.Version_6_0, "Blazor", new List<RazorExtension>()), fileSystem, b =>
            {
                b.SetRootNamespace("CompileTimeBlazor");
            ...
            var a = engine.ProjectFeatures.ToList();
            var b = engine.Phases.ToList();
            var c = engine.EngineFeatures.ToList();

            CompileLog.Add("Create file");
            var file = new MemoryRazorProjectItem(code, true, "", "/Test.razor");
